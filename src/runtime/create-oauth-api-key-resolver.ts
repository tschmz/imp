...
type StoredOAuthCredentials = OAuthCredentials & {
  type?: string;
};
...
export function createOAuthApiKeyResolver(
  ...
  return async (provider) => {
    ...
    let auth: Record<string, StoredOAuthCredentials>;
    try {
...
async function loadAuthFile(
  ...
  try {
    ...
    return isRecord(parsed) ? (parsed as Record<string, StoredOAuthCredentials>) : {};
  } catch (error) {

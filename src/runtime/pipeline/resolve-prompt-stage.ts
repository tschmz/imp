...
export async function resolvePromptStage(
  ...
  const templateContext = createPromptTemplateContext({
    ...
    invocation: context.input.runtime?.invocation,
    ingress: context.input.runtime?.ingress,
    output: context.input.runtime?.output,
...
function resolvePromptWorkingDirectory(
  ...
  return conversation.state.workingDirectory ?? agent.workspace?.cwd ?? agent.home;
}
